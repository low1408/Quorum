...
import { validateCouncilContext, type CouncilContext } from '../from_orchestrator/mcp/contextValidation.ts';
...
const jsonOutputPath = path.join(outputDir, 'council-lifecycle.context.json');
...
function buildMcpContext(testRun: { command: string; output: string; status: number | null }): CouncilContext {
  const files = [...fullImplementationFiles, ...supportingFiles, ...testFiles].map((relativePath, index) => {
    const content = sanitizeText(readRepoFile(relativePath));
    const role = fullImplementationFiles.includes(relativePath)
      ? 'core'
      : testFiles.includes(relativePath)
        ? 'test'
        : 'contract';
    const lines = content.split(/\r?\n/u).length;
    return {
      path: relativePath,
      content,
      relevance: `${role} evidence for the consult_council lifecycle review.`,
      start_line: 1,
      end_line: lines,
      total_lines: lines,
      is_excerpt: false
  ...
  return {
    ...
    notes: 'Generated by npm run review-context. Source content is sanitized before serialization and validated through validateCouncilContext.',
    files,
    evidence_manifest: files.map((file, index) => ({
      id: `EV${String(index + 1).padStart(3, '0')}`,
      path: file.path,
      sha256: sha256(file.content),
      role: file.path.startsWith('tests/') ? 'test' as const : fullImplementationFiles.includes(file.path) ? 'core' as const : 'contract' as const,
    ...
    structured_review: {
      review_objective: 'Review the consult_council request lifecycle for validation, concurrency limits, timeout and cancellation behavior, retry policy, resource cleanup, privacy controls, and persistence consistency.',
      architecture: 'MCP server validates consult_council input, validateCouncilContext normalizes and checks context, runCouncilConsultation executes provider analyses and consolidation, and report artifacts persist the final anonymous report.',
      execution_flow: 'consult_council -> initSchema -> validateCouncilContext -> runCouncilConsultation -> ProviderSessionPool.acquire -> OrchestrationRunner.executeTask -> consolidation -> structured MCP response.',
      assumptions_and_invariants: 'Generated evidence is local and mock/test based; terminal statuses should not be overwritten; retries only apply to retryable pre-dispatch failures; context validation rejects unsafe paths, stale hashes, binary content, and secret-bearing payloads.',
      core_evidence: fullImplementationFiles.join(', '),
      supporting_contracts: supportingFiles.join(', '),
      privacy_and_persistence: 'Context validation, security/session files, database excerpts in Markdown, and .env.example provide privacy and persistence evidence. Real .env files, sessions, production databases, logs, and browser state are omitted.',
      tests_and_runtime_evidence: `Command: ${testRun.command}; exit_status=${testRun.status}; sanitized output digest=${sha256(testRun.output)}.`,
      omitted_material: 'Provider-specific adapter bodies, workflow compiler/DAG/debate/summarization/evaluation modules, node_modules, lockfile contents, local database files, session storage, logs, real env files, and live provider traces are omitted.'
    }
  };
}

function buildBundle(): { markdown: string; mcpContext: CouncilContext; testStatus: number | null } {
  const testRun = runTests();
  const mcpContext = buildMcpContext(testRun);
  const blocks: string[] = [];
...
function validateBundle(markdown: string, mcpContext: CouncilContext, testStatus: number | null): void {
  ...
  validateCouncilContext(mcpContext, mcpContext.structured_review?.review_objective || 'Review generated context.');
}

function main(): void {
  fs.mkdirSync(outputDir, { recursive: true });
  const { markdown, mcpContext, testStatus } = buildBundle();
  fs.writeFileSync(outputPath, markdown, 'utf8');
  fs.writeFileSync(jsonOutputPath, `${JSON.stringify(mcpContext, null, 2)}\n`, 'utf8');

  if (validateOnly) {
    validateBundle(markdown, mcpContext, testStatus);
    console.log(`Generated and validated ${relPath(outputPath)} and ${relPath(jsonOutputPath)}`);
  } else {
    console.log(`Generated ${relPath(outputPath)} and ${relPath(jsonOutputPath)}`);
  }
}
